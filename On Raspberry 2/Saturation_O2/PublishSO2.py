...
import json
import os
from generatorSP import ReadingSP
import dweepy

class PublishSO2(object):
    ...
    def load_topic(self):
        try:
            ...
            self.so2_topic = json_format['topic']['Saturation_O2']
        except:
    ...
    def myPublish(self):
        try:
            ...
            msg_info = self._paho_mqtt.publish(self.so2_topic, str(json_data), qos=1)
            msg_info.wait_for_publish()
            if msg_info.is_published() == True:
                print("\n\nMessage is published.")
            print('id :'+patient_id+'\npatient :'+self.patient+'\nName :'+self.name+'\nSurname :'+self.surname+'\nAge :'+self.age+'\nbn :'+data["bn"]+'\ne :'+str(data['e']))
            dweepy.dweet_for('progiotdash2_o2', data['e'])
...
if __name__ == '__main__':
    ...
    Publisher = PublishSO2(url, device_data, sensor, patient_data, broker_ip, broker_port)
