...
import json
import os
from generatorGL import ReadingGLU
import dweepy

'''Publisher of Senml data about Glucose Level'''
class PublishGLU(object):
    ...
    def load_topic(self):
        try:
            ...
            self.glu_topic = json_format['topic']['Glucose_Concentration']
        except:
    ...
    def myPublish(self):
        try:
            ...
            msg_info = self._paho_mqtt.publish(self.glu_topic, str(json_data), qos=1)
            msg_info.wait_for_publish()
            if msg_info.is_published() == True:
                print("\n\nMessage is published.")
            print('id :'+patient_id+'\npatient :'+self.patient+'\nName :'+self.name+'\nSurname :'+self.surname+'\nAge :'+self.age+'\nbn :'+data["bn"]+'\ne :'+str(data['e']))
            dweepy.dweet_for('progiotdash2_glu', data['e'])
        except:
            current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
if __name__ == '__main__':
    ...
    Publisher = PublishGLU(url, device_data, sensor, patient_data, broker_ip, broker_port)
