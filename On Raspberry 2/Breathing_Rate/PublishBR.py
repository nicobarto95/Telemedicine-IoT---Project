...
import json
import os
from gen_resp import ReadingBreathingRate
import dweepy

class PublishBR(object):
    ...
    def load_topic(self):
        try:
            ...
            self.br_topic = json_format['topic']['Breathing_Rate']
        except:
    ...
    def myPublish(self):
        try:
            ...
            msg_info = self._paho_mqtt.publish(self.br_topic, str(json_data), qos=1)
            msg_info.wait_for_publish()
            if msg_info.is_published() == True:
                print("\n\nMessage is published.")
            print('id :' + patient_id + '\npatient :' + self.patient + '\nName :' + self.name + '\nSurname :' + self.surname + '\nAge :' + self.age + '\nbn :' +
                data["bn"] + '\ne :' + str(data['e']))
            dweepy.dweet_for('progiotdash2_br', data['e'])
        except:
            current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
if __name__ == '__main__':
    ...
    Publisher = PublishBR(url, device_data, sensor, patient_data, broker_ip, broker_port)
