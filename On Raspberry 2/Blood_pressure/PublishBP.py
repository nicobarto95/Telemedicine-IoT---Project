...
import json
import os
from generatorBP import ReadingBP
import dweepy

class PublishBP(object):
    ...
    def load_topic(self):
        try:
            ...
            self.bp_topic = json_format['topic']['Blood_Pressure']
        except:
    ...
    def myPublish(self):
        ...
        msg_info = self._paho_mqtt.publish(self.bp_topic, str(json_data), qos=1)
        msg_info.wait_for_publish()
        if msg_info.is_published() == True:
            print("\n\nMessage is published.")
        print('id :' + patient_id + '\npatient :' + self.patient + '\nName :' + self.name + '\nSurname :' + self.surname + '\nAge :' + self.age + '\nbn :' +data["bn"] + '\ne :' + str(data['e']))
        dweepy.dweet_for('progiotdash2_bp', data['e'])
        '''except:
            current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
if __name__ == '__main__':
    ...
    Publisher = PublishBP(url, device_data, sensor, patient_data, broker_ip, broker_port)
