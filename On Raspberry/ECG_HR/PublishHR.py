...
import json
import os
from gen_ECG import ReadingHR
import dweepy

class PublishHR(object):
    ...
    def load_topic(self):
        try:
            ...
            self.hr_topic = json_format['topic']['Heart_Rate']
        except:
    ...
    def myPublish(self):
        try:
            ...
            msg_info = self._paho_mqtt.publish(self.hr_topic, str(json_data), qos=1)
            msg_info.wait_for_publish()
            if msg_info.is_published() == True:
                print("\n\nMessage is published.")
            print('id :'+patient_id+'\npatient :'+self.patient+'\nName :'+self.name+'\nSurname :'+self.surname+'\nAge :'+self.age+'\nbn :'+data["bn"]+'\ne :'+str(data['e']))
            dweepy.dweet_for('progiotdash1_hr', data['e'])
...
if __name__ == '__main__':
    ...
    Publisher = PublishHR(url, device_data, sensor, patient_data, broker_ip, broker_port)
