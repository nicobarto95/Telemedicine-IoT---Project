...
import json
import os
from generatorSMOKER import ReadingSmoker
import dweepy

class PublishSmoker(object):
    ...
    def load_topic(self):
        try:
            ...
            self.smoke_topic = json_format['topic']['Smoker']
        except:
    ...
    def myPublish(self):
        ...
        msg_info = self._paho_mqtt.publish(self.smoke_topic, str(json_data), qos=1)
        msg_info.wait_for_publish()
        if msg_info.is_published() == True:
            print("\n\nMessage is published.")
        print('id :' + patient_id + '\npatient :' + self.patient + '\nName :' + self.name + '\nSurname :' + self.surname + '\nAge :' + self.age + '\nbn :' +data["bn"] + '\ne :' + str(data['e']))
        dweepy.dweet_for('progiotdash1_smo', data['e'])
...
if __name__ == '__main__':
    try:
        with open("../Smoker/config_file.json", 'r') as file:
    ...
    patient_data = {"patient_id": str(patient_id),
                    "patient": patient,
                    "name": name,
                    "surname": surname,
                    "age": age}
    ...
    Publisher = PublishSmoker(url, device_data, sensor, patient_data, broker_ip, broker_port)
